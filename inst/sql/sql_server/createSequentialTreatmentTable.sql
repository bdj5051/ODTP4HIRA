...
FROM @cohortDatabaseSchema.@cohortTable_OSTEOPOROSIS_PATHWAY_ANALYSIS_EVENTS T1
...
FROM (
	...
	(SELECT SUBJECT_ID, DRUG_CONCEPT_ID, COHORT_START_DATE, DAYS_SUPPLY, DURATION, RIGHT(COHORT_DEFINITION_ID, 1) AS DRUG_ID FROM @cohortDatabaseSchema.@cohortTable_DRUG) T2
...
FROM (
	...
	(SELECT SUBJECT_ID, DRUG_CONCEPT_ID, COHORT_START_DATE, DAYS_SUPPLY, DURATION, RIGHT(COHORT_DEFINITION_ID, 1) FROM @cohortDatabaseSchema.@cohortTable_DRUG) T2
WHERE T1.SUBJECT_ID = T2.SUBJECT_ID AND T1.LINE_START_DATE <= T2.COHORT_START_DATE AND T1.COHORT_ID=T2.DRUG_ID;
...
FROM (
	...
	FROM @cohortDatabaseSchema.@cohortTable_DRUG) T2
...
FROM (
	...
	(SELECT SUBJECT_ID, DRUG_CONCEPT_ID, COHORT_START_DATE, DAYS_SUPPLY, DURATION, RIGHT(COHORT_DEFINITION_ID, 1) RIGHT(COHORT_DEFINITION_ID, 1) AS DRUG_ID FROM @cohortDatabaseSchema.@cohortTable_DRUG) T2

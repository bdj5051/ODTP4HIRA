...
IF OBJECT_ID('HIRA_CDM_YU_OSTEO.OSTEO_SUBJECTS_EVENTS', 'U') IS NOT NULL
	DROP TABLE HIRA_CDM_YU_OSTEO.OSTEO_SUBJECTS_EVENTS;

CREATE TABLE HIRA_CDM_YU_OSTEO.OSTEO_SUBJECTS_EVENTS (
	SUBJECT_ID BIGINT,
	COMBO_ID INT,
...
INSERT INTO HIRA_CDM_YU_OSTEO.OSTEO_SUBJECTS_EVENTS (
	SUBJECT_ID,
	COMBO_ID,
	FIRST_START_DATE,
	SECOND_COMBO,
...
SELECT T1.SUBJECT_ID, T1.COMBO_ID AS FIRST_COMBO
     , T1.COHORT_START_DATE AS FIRST_START_DATE
     , NULL AS SECOND_COMBO
     , NULL AS SECOND_START_DATE
     , NULL AS THIRD_COMBO
     , NULL AS THIRD_START_DATE
FROM HIRA_CDM_YU_OSTEO.OSTEO_PATHWAY_EVENTS T1
JOIN #ODTP_SUBJECTS T2 on T2.SUBJECT_ID=T1.SUBJECT_ID
WHERE PATHWAY_ANALYSIS_GENERATION_ID=1 AND T2.CNT=1;
...
SELECT SUB.*
FROM (
	SELECT T1.SUBJECT_ID, T1.COMBO_ID AS FIRST_COMBO, T1.COHORT_START_DATE AS FIRST_START_DATE,
	  LEAD(T1.COMBO_ID) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY T1.COHORT_START_DATE) AS SECOND_COMBO,
	  LEAD(T1.COHORT_START_DATE) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY T1.COHORT_START_DATE) AS SECOND_START_DATE
	  , NULL AS THIRD_COMBO
	  , NULL AS THIRD_START_DATE
	FROM HIRA_CDM_YU_OSTEO.OSTEO_PATHWAY_EVENTS T1
	JOIN #ODTP_SUBJECTS T2 on T2.SUBJECT_ID=T1.SUBJECT_ID
...
WHERE SUB.SECOND_COMBO IS NOT NULL;
...
SELECT T1.SUBJECT_ID, T1.COMBO_ID AS FIRST_COMBO, T1.COHORT_START_DATE AS FIRST_START_DATE,
  LEAD(T1.COMBO_ID) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY T1.COHORT_START_DATE) AS SECOND_COMBO,
  LEAD(T1.COHORT_START_DATE) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY T1.COHORT_START_DATE) AS SECOND_START_DATE,
  LEAD(T1.COMBO_ID,2) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY T1.COHORT_START_DATE) AS THIRD_COMBO,
  LEAD(T1.COHORT_START_DATE,2) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY T1.COHORT_START_DATE) AS THIRD_START_DATE
FROM HIRA_CDM_YU_OSTEO.OSTEO_PATHWAY_EVENTS T1
JOIN #ODTP_SUBJECTS T2 on T2.SUBJECT_ID=T1.SUBJECT_ID
WHERE PATHWAY_ANALYSIS_GENERATION_ID=1 AND T2.CNT=3  
) SUB
WHERE SUB.THIRD_COMBO IS NOT NULL;


-- ** CREATE Prescription TABLE **
IF OBJECT_ID('HIRA_CDM_YU_OSTEO.OSTEO_PRESCRIPTION_EVENTS', 'U') IS NOT NULL
	DROP TABLE HIRA_CDM_YU_OSTEO.OSTEO_PRESCRIPTION_EVENTS;

CREATE TABLE HIRA_CDM_YU_OSTEO.OSTEO_PRESCRIPTION_EVENTS (
	LINE INT,
	COMBO INT,
...
INSERT INTO HIRA_CDM_YU_OSTEO.OSTEO_PRESCRIPTION_EVENTS (
	LINE,
	COMBO,
...
FROM (
		...
		FROM HIRA_CDM_YU_OSTEO.OSTEO_SUBJECTS_EVENTS SUBJECTS
		JOIN HIRA_CDM_YU_OSTEO.OSTEO COHORTS
	...
	(SELECT SUBJECT_ID, DRUG_CONCEPT_ID, COHORT_START_DATE, DAYS_SUPPLY, DURATION, RIGHT(COHORT_DEFINITION_ID, 1) AS DRUG_ID FROM HIRA_CDM_YU_OSTEO.OSTEO_DRUG) T2
...
FROM (
		SELECT COHORTS.COHORT_DEFINITION_ID
			, RIGHT(COHORTS.COHORT_DEFINITION_ID, 1) AS COHORT_ID
			, SUBJECTS.SUBJECT_ID
			, SUBJECTS.SECOND_START_DATE AS LINE_START_DATE
		...
		JOIN @cdmDatabaseSchema.OBSERVATION_PERIOD OP
		ON SUBJECTS.SUBJECT_ID=OP.PERSON_ID
		WHERE COHORTS.COHORT_DEFINITION_ID IN (1001,1002,1003,1004,1005,1006) 
	      	  AND SUBJECTS.SECOND_COMBO IN (1,2,4,8,16,32) AND SUBJECTS.THIRD_COMBO IS NOT NULL
	) T1,
	(SELECT SUBJECT_ID, DRUG_CONCEPT_ID, COHORT_START_DATE, DAYS_SUPPLY, DURATION, RIGHT(COHORT_DEFINITION_ID, 1) AS DRUG_ID 
	FROM HIRA_CDM_YU_OSTEO.OSTEO_DRUG) T2
WHERE T1.SUBJECT_ID = T2.SUBJECT_ID 
  AND T2.COHORT_START_DATE >= T1.LINE_START_DATE 
  AND T2.COHORT_START_DATE < T1.LINE_END_DATE;

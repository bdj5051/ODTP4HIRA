...
WHERE (YEAR(E.start_date) - P.year_of_birth >= 50 and YEAR(E.start_date) - P.year_of_birth <= 64)
GROUP BY e.person_id, e.event_id
...
(
  ...
  (
    ...
    (
      --cteEndDates
      select PERSON_ID, DATEADD(day,-1 * 120,EVENT_DATE) as END_DATE -- unpad the end date by 120
      FROM
      (
				...
				(
					...
					-- add the end dates with NULL as the row number, padding the end dates by 120 to allow a grace period for overlapping ranges.
					select PERSON_ID, DATEADD(day,120,DRUG_EXPOSURE_END_DATE), 1 as EVENT_TYPE, NULL
					FROM #drugTarget D
				) RAWDATA
      ) E

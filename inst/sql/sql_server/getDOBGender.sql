SELECT P.PERSON_ID AS SUBJECT_ID
	 ...
	 , DATEFROMPARTS(YEAR(OP.OBSERVATION_PERIOD_START_DATE), MONTH(OP.OBSERVATION_PERIOD_START_DATE),1) AS START_YEARMTH
	 , DATEFROMPARTS(YEAR(OP.OBSERVATION_PERIOD_END_DATE), MONTH(OP.OBSERVATION_PERIOD_END_DATE),1) AS END_YEARMTH
FROM (SELECT * 
	  FROM @cdm_database_schema.PERSON PT
	  JOIN @cohort_database_schema.@cohort_table CT 
	  ON PT.PERSON_ID=CT.SUBJECT_ID
	  WHERE CT.COHORT_DEFINITION_ID IN (2001,2002,2003,2004,2005)) P -- CohortType:DRUG
